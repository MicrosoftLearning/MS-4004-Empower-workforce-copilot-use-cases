...
1. Open the **Microsoft Copilot** app in a new browser tab. In the list of agents in the left navigation pane, select **Analyst**.

2. In the **Analyst** agent's prompt field, attach the **Market Insights Report – Smart Workspaces.docx** file that you saved to your OneDrive in the prior task.
...
4. Review the response. You want to eventually save this content to a Word document, so select the ellipsis **(...)**, then select the **Edit in Pages** option from the drop-down menu.

5. When you select **Edit in Pages**, Copilot displays two panes—one that displays the **Analyst** agent and your current chat, and another that displays the **Pages** form and the content that was copied from your chat.

    Review the content in the **Pages** form. Delete any extraneous text that was copied and pasted with the results (see the start and end of the content). You can make any changes to the headings or any other content on the form.

6. In the **Analyst** chat pane, ask the agent to create a visual summary (infographic) of these priorities.
...
12. Review the results. You want to eventually save this content to a PDF document, so select the ellipsis **(...)** next to the agent's response, then select the **Edit in Pages** option from the drop-down menu.
...
15. Once the agent finishes generating the graphic, select the ellipsis **(...)** next to the agent's response, then select **+ Add to page**. Scroll to the bottom of the **Pages** form and verify the graphic was inserted. Delete any extraneous text that was copied and pasted with the graphic (see the start and end of the response containing the graphic).

16. You're now ready to save this Pages content to a PDF file. On the **Pages** pane, select the ellipsis (**...**) that appears at the top of the pane, then select **Export** > **PDF**.

17. Once the agent finishes generating the PDF, the PDF opens in a new browser tab. Select the **Printer** drop-down, then select **Save as PDF**. Select the **Save** button that appears at the bottom of the **Print** pane. Save the PDF to your OneDrive as **Smart Workspace opportunities.pdf**.
...
20. Review the results. You want to eventually save this content as a PowerPoint presentation, so select the ellipsis **(...)** next to the agent's response, then select the **Edit in Pages** option from the drop-down menu that appears.

21. On the **Analyst** chat pane, ask the Analyst agent to create a PowerPoint presentation.

22. The agent generates the presentation directly. When it finishes, it displays a confirmation message (for example, "I've created a clean, executive-ready presentation for you covering messaging, channels, partnerships, and risks") and provides a link to download the presentation file.

23. Optionally, the agent might offer to refine the presentation further—for example, by adding visual frameworks (impact matrix, funnel, personas), tailoring it to a specific audience, or redesigning it with a stronger storytelling flow and speaker notes. If you want, ask the agent to apply any of these refinements.

24. To review and edit the slides, select the presentation file link (you may need to select the number next to the link) to download the file, then open it in PowerPoint. You can then review the slides and make any necessary updates.
