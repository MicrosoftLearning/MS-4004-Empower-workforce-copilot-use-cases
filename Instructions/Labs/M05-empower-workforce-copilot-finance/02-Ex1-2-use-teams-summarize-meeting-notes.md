...
1. Select the following link to download the [**Fabrikam Finance meeting notes.txt**](https://go.microsoft.com/fwlink/?linkid=2347810) file. Store the file in your OneDrive account for use by Copilot in your tenant.
  ...
  > [!NOTE]
  > You can copy and paste the contents of the downloaded meeting notes file into Notepad or any text editor of your choice.

2. In your web browser, go to the **Microsoft Copilot** home page, select the **App Launcher** (grid icon) in the top-left corner, select **More Apps**, and then select **Teams** from the list of available apps.

3. In **Teams for the web**, select **Copilot** in the left navigation pane.

4. On the **Copilot Chat** page, ask Copilot to summarize the key decisions, updates, and action items in the **Fabrikam Finance meeting notes.txt** file. You should attach the file from OneDrive before submitting your prompt.

5. Review the summary to ensure it includes decisions, next steps, and responsibilities. Ask Copilot to generate a downloadable file for distribution to the meeting participants. After Copilot generates the document, close the preview pane, select the ellipsis **(...)** next to the document, and then select **Download**.
