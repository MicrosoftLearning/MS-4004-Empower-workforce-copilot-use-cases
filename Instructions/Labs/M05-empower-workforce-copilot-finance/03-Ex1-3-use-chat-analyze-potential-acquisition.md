...
1. Select the following link to download the [**Relecloud Business Perspective.docx**](https://go.microsoft.com/fwlink/?linkid=2347813) file. Store it in your OneDrive account for use by Copilot in your tenant.

2. In your web browser, go to the **Microsoft Copilot** home page. Since this task involves reviewing a file uploaded to OneDrive and generating insights from that internal document, turn on **Work IQ**. When **Work IQ** is turned off, **Web** mode is active and searches external sources such as public websites and blogs, so it doesn't apply to this task.
...
4. Upon reviewing the results, you feel that Copilot's summary was a good start, but it didn't provide the level of detail necessary for a comprehensive analysis of Relecloud's business. In reviewing your previous prompt, you notice that you asked for a summary containing three sections but didn't provide any details about what you wanted Copilot to include in those sections. When you leave those types of decisions to Copilot, its results might not always meet your expectations.

    This time, ask Copilot to create an expanded version of the previous report. Tell it to retain all the information from the previous summary and add the following details:
...
5. Review the results. Note the difference between Copilot's first summary report, which was based on a high-level prompt, and the second summary report, which was based on a much more detailed request. This exercise shows the importance of a detailed prompt that includes the four key elements of an effective prompt.
