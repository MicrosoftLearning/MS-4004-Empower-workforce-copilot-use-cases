---
lab:
  title: 'Exercise 2, Task 1: Use Copilot Chat to evaluate vendor contracts'
...
5. Review the comparison generated by Copilot. At the end of the response, select the ellipsis (**...**) next to the response and then select the **Edit in Pages** option.

6. When editing in **Pages**, note how Copilot displays the Copilot chat pane along with the Pages form. In the **Pages** form, select the ellipsis **(...)** in the top-right corner and then select  **Export** > **Document**. In the dialog box that appears, select **Open Word**, which opens the document in **Word for the web**.

7. In **Word for the web**, Copilot copies its entire response into a document, including the extraneous chat content that appeared at the beginning and end of the chat. Delete any extraneous text that was pasted in as well. Select the file name that appears above the menu bar and change the file name to **Smart Sensor contract comparison**. Leave the document open as you plan to add additional information to it in the remaining steps.

8. In the Word document, select the **Copilot** option in the bottom-right corner of the document. In the Copilot pane, verify that the mode is set to **Allow editing**. If it shows **Chat only**, select the dropdown above the prompt box and then select **Allow editing** so that Copilot can edit your document directly.
...
13. Review the new section that Copilot just added to the document. If it appears, select **Done** to accept the changes. You plan to create a PowerPoint presentation based on this document in the next task, so feel free to add any extra enhancements that you want. For example, if time permits, you can add a section that compares another topic from the earlier list. Or, you can review Copilot's suggested prompts at the end of the Copilot pane and submit any requests that are of interest to you.
