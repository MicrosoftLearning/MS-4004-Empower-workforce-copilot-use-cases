...
2.  In your Microsoft Edge browser, go to the **Microsoft 365** home page **(https://www.office.com)** and **select the Work option, which is used** for searching and analyzing internal organizational content (OneDrive, SharePoint, Teams, Outlook).
...
4.  Review the comparison generated by Copilot. You want to save it to a Word document, so select the **Copy** icon that appears after the response. Then open a blank document in Word and paste in the response. Delete any extraneous text that was copied and pasted in from the chat session. Save the document as **Smart Sensor contract comparison**. Leave the document open as you plan to add additional information to it in the remaining steps.

5.  Navigate back to your Copilot Chat session. Ask Copilot which contract has more favorable terms for Fabrikam, and why? Also ask where Fabrikam should focus its negotiation efforts.

6.  Copy the response and paste it at the end of the **Smart Sensor contract comparison** document. Delete any extraneous text that was copied and pasted in from the chat session (typically at the start and end of the copied text).

7.  Return to the Copilot Chat session. When financial analysts review supplier contracts in the real world, there are numerous areas they might consider including in their analysis. Typical areas include:
...
8.  Instead of asking Copilot to compare the contracts across several areas at once, select just one area that’s of interest to you and ask Copilot to compare the contracts specifically on that point. Focusing on a single aspect can often yield more thorough and precise results than overloading Copilot with multiple requests in a single prompt.

9.  Copy the response and paste it at the end of the **Smart Sensor contract comparison** document. Delete any extraneous text that was copied and pasted in from the chat session.

10.  Repeat the prior step and ask Copilot to compare the contracts for one other area that’s of interest to you.
